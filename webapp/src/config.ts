export const H5P_PUBLIC_URL = process.env.NEXT_PUBLIC_H5P_URL
export const WEBAPP_URL = process.env.NEXT_PUBLIC_WEBAPP_URL
export const OLLAMA_URL = process.env.OLLAMA_URL;
export const NEXT_PUBLIC_SERVER_URL = process.env.NEXT_PUBLIC_SERVER_URL;
export const DEFAULT_PAGE_NUMBER = 50
export const EMPTY_DOCUMENT = { "type": "doc", "content": [ { "type": "heading", "attrs": { "level": 1 } } ] }
export const PROJECT_NAME = 'Ada'

export const userColors = ['#fb7185', '#fdba74', '#d9f99d', '#a7f3d0', '#a5f3fc', '#a5b4fc', '#f0abfc']
export const themeColors = ['#000000', '#3498db', '#2ecc71', '#e74c3c', '#f39c12', '#9b59b6', '#1abc9c', '#ffff00']
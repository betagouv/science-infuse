...
const RenderPdfTextCard = (props: { searchWords: string[], chunk: ChunkWithScore<"pdf_text"> }) => {
    return <Card
        ...
        footer={<ul className="fr-btns-group fr-btns-group--inline-reverse fr-btns-group--inline-lg">
            <li>
                <button className="fr-btn fr-btn--secondary" ><a target="_blank" href={`${NEXT_PUBLIC_FILE_SERVER_URL}${props.chunk.document.public_path}#page=${props.chunk.metadata.page_number}`}>source</a></button>
            </li>
        </ul>}
        size="small"
        title=""
...
const RenderPdfImageCard = (props: { chunk: ChunkWithScore<"pdf_image"> }) => {
    return <Card
        background
        border
        desc={<img className="w-full" src={`${NEXT_PUBLIC_FILE_SERVER_URL}${props.chunk.metadata.public_path}`} />}
        enlargeLink
        linkProps={{
...
export const RenderVideoTranscriptCard = (props: { chunk: ChunkWithScore<"video_transcript">, searchWords: string[] }) => {
    ...
    return <Card
        ...
        desc={
            <div>
                <VideoPlayer videoUrl={`${NEXT_PUBLIC_FILE_SERVER_URL}${props.chunk.document.public_path}`} startOffset={props.chunk.metadata.start} endOffset={props.chunk.metadata.end} />
                <Highlighter
                    highlightClassName="highlightSearch"
                    searchWords={props.searchWords}
                    autoEscape={false}

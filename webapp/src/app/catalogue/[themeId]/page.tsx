...
import { BlockResults, ChapterResults } from "@/app/recherche/page";
...
const Filters: React.FC<FiltersProps> = ({ selectedFilter, onFilterChange }) => {
    const filters = ['Tous', '5ème', '4ème', '3ème']
        ...
        <div className="flex flex-wrap justify-center items-center w-full  border-0 border-white p-4">
            ...
            <div className="flex flex-wrap">
                {filters.map((filter) => (
                    <button
                        key={filter}
                        className={`px-4 py-[1.5rem] m-0 text-sm hover:text-black ${
...
const Catalogue: React.FC<CatalogueProps> = ({ params }) => {
    ...
    const [chapters, setChapters] = useState<ChapterWithBlock[]>([]);

    useEffect(() => {
        const fetchChapters = async () => {
            try {
                const response = await apiClient.getChaptersByTheme(themeId);
        ...
        <div className="w-full fr-grid-row fr-grid-row--center">
            <div className="fr-col-12 fr-container main-content-item">
                <div className="py-16 flex flex-col gap-8 md:px-0">
                    ...
                    <ChapterResults chapters={chapters} />
                </div>
            </div>
        </div>
    )

    ...
    if (blocks && blocks.length > 0) {
        for (let i = 0; i < blocks.length; i++) {
            ...
            if (block.attrs && block.attrs.id && block.content) {
                ...
                try {
                    const saved = await apiClient.saveBlock(blockId, blockTitle, block.content)
                    if (!saved) {
                        return false;
                    }
                } catch (error) {
...
export const handleSave = async (editor: Editor, chapterId: string, title: string, content: JSONContent): Promise<HandleSaveResponse> => {

    let saveChapterOk;
    try {
        saveChapterOk = await apiClient.saveChapter(chapterId, title, content);
    } catch (error) {
        saveChapterOk = false;
    }
